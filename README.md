# Hi, I'm Vikrant Desai 👋

🎯 **Aspiring Data Analyst / Data Scientist / Data Engineer / Data Specialist**
📍 India

---

## 🚀 About Me

Passionate data professional with strong foundation in **Statistics, Machine Learning, Deep Learning, and Data Engineering concepts**. Experienced in developing end‑to‑end ML workflows, data cleaning, predictive modeling, and data visualization.

Currently working on advanced analytics projects and continuously expanding skills in real‑world data applications.

---

## 📌 Skills & Tools

### **Programming & Database**

* Python | SQL

### **Machine Learning / Deep Learning**


### **Data Analysis & Visualization**

* Excel | Power BI | Tableau | Matplotlib | Seaborn

### **Statistical Tools**

* SPSS | Minitab

---

## 📂 Featured Projects

|             Project              |                      Description                  |       Tools         | Link |
|----------------------------------|---------------------------------------------------|---------------------|------|
| Loan Approval Prediction         | ML model for automated loan approval              | Python, ML, Sklearn |  🔗  |
| Credit Card Default Prediction   | Classification model to predict defaults          | Python, ML, EDA     |  🔗  |
| Customer Churn Prediction        | Identified churn drivers & built predictive model | Python, ML, SHAP    |  🔗  |
| Wine Quality Prediction          | Regression model to predict wine rating           | Python, ML          |  🔗  |
| Car Rental System                | Command-line system with Python & file handling   | Python, OOP         |  🔗  |
| SQL + Python Music Data          | Data analysis + SQL integration                   | Python, MySQL       |  🔗  |
| CRUD in MySQL                    | Full CRUD using Python                            | Python, MySQL       |  🔗  |
| Command-line Analytics Tool      | CLI tool for data exploration                     | Python              |  🔗  |
| Superstore Analysis              | BI & insights from retail data                    | Excel / Python      |  🔗  |
| Blinkit Dashboard                | Sales analytics dashboard                         | PowerBI             |  🔗  |

> *Pinned projects will be updated with direct repository links.*

---

## 🏆 Highlights

* 📊 Strong statistical foundation
* 🤖 Hands‑on ML project experience
* 🧠 Trained in Python, Analytics & AI tools
* 🎓 Completed Master's in Applied Statistics (2025)
* 📈 Experienced Data Science intern

---

## 📬 Connect With Me

* 🔗 LinkedIn: linkedin.com/in/vikrant-desai-725803290
* 📧 Email: desaivikrant276@gmail.com
* 💼 Portfolio Website (Coming soon)

---

⭐ *Thanks for visiting! Feel free to explore my work & connect!*
